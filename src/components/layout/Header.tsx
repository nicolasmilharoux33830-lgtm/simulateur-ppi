      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center gap-6" aria-label="Navigation principale">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-ink/70 hover:text-pine font-body text-sm font-medium transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link
              href="/connexion"
              className="bg-moss text-white hover:bg-pine px-4 py-2 rounded-lg text-sm font-display font-semibold transition-colors"
            >
              Connexion
            </Link>
          </div>
          ...
          {/* Mobile menu button */}
          <button
            className="md:hidden p-2 text-ink rounded-lg hover:bg-paper transition-colors"
          ...
          <nav className="flex flex-col py-4 px-4 gap-1" aria-label="Navigation mobile">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-3 py-2 text-ink/70 hover:text-pine hover:bg-paper rounded-lg font-body text-sm font-medium transition-colors"
            ...
            <div className="pt-3 border-t border-ink/10 mt-3">
              <Link
                href="/connexion"
                className="block text-center bg-moss text-white hover:bg-pine px-4 py-2 rounded-lg text-sm font-display font-semibold transition-colors"
                onClick={() => setOpen(false)}
              >
                Connexion
              </Link>
